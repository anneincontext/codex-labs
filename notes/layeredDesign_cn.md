...
3. 通过 `codex-api` 调 **OpenAI Responses API**；经由支持 SSE 和 WebSocket 的路径消费流式模型输出
...
**角色：** 和 **OpenAI Responses API** 的**网络层**——请求模型、SSE 解析、支持 WebSocket 的流式路径、重试、鉴权头。
...
| `codex-api` | Responses / Compact / Memory 等 API 的请求体、SSE 解析、`ResponseEvent` 流类型 |

core 拼好 `instructions`、`input`、`tools` 后，交给这一层；收回来的是结构化的 `ResponseEvent`（文本增量、function call、完成信号等）。

**这一层的价值：** 把「和 OpenAI 怎么说话」从业务逻辑里剥离。换 endpoint、加重试、改 SSE 解析和流式传输细节，主要动这一层和 core model client，不动 session / turn 逻辑。

---

## ⑤ Execution — core 执行路径 + `exec-server`

**角色：** **真正在机器上跑进程**的地方——shell、子进程生命周期、stdin/stdout。

默认情况下这发生在 **`codex-core` 内部**：`core/src/exec.rs`（`process_exec_tool_call` → `spawn_child_async`）直接 spawn 进程，外面包一层沙箱（⑥）。core 决定「要跑 `cargo test`」，core 执行路径负责 spawn 并把输出流回来。

[`exec-server`](https://github.com/openai/codex/tree/main/codex-rs/exec-server) 是把这段执行**抽成独立服务的实验性（experimental）**能力，用于两类场景：

- 本地：`codex exec-server`（标记为 `[EXPERIMENTAL]`），WebSocket 上 JSON-RPC
- 远程：注册到环境 registry，通过 **Noise 加密 relay** 跨机器控制进程（app-server 和 exec-server 可以在不同 OS 上）

**这一层的价值：** 起子进程是个独立的关注点，有自己的沙箱包装；把它抽成 `exec-server` 后，同一套执行既能本地跑也能跨机器跑，还能单独加固。
...
策略来自 `SandboxPolicy` / permissions 配置（只读、工作区可写、网络规则等）。core 执行路径起命令前，会按策略包一层沙箱。
...
**角色：** 给 **富客户端**用的**对外 API 层**（VS Code 插件、桌面 App、Python SDK）。
...
`app-server-protocol` 定义 v1/v2 API 类型，并可生成 TypeScript schema，保证 IDE 和 CLI 版本对齐。

**这一层的价值：** 把 core 包装成**稳定、可版本化的 RPC 接口**，让编辑器和 app-server 客户端不用嵌入 Rust TUI。TypeScript SDK 是例外：它包装的是 `codex exec --experimental-json` 的 stdin/stdout JSONL 接口，而不是 app-server。
...
4. codex-core（③）
      ...
      │  模型要跑测试 → ToolRouter
      ▼
5. core 执行路径（⑤）  spawn shell（core/src/exec.rs）
      │  外包一层 sandboxing（⑥）
      ▼
...
| Execution | 跑子进程——默认在 core 内；`exec-server`（实验性）用于本地/远程 |
