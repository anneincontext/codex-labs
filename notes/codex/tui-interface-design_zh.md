...
> **回答什么：** TUI crate 的接口设计——事件总线、app-server 桥、渲染模型。
...
[`codex-tui`](https://github.com/openai/codex/tree/main/codex-rs/tui) crate 的结构说明：分层、事件总线、渲染 trait、输入路由。配合 [architecture_zh.md](architecture_zh.md) 的仓库地图和 [layered-design_zh.md](layered-design_zh.md) 的分层阅读。
...
| 你想查的 | 官方文档 | 仓库内源码 / 笔记 |
| -------- | -------- | ----------------- |
| **App Server JSON-RPC**（thread/turn、审批、事件流） | [Codex App Server](https://developers.openai.com/codex/app-server) · 详尽版 [app-server README](https://github.com/openai/codex/blob/main/codex-rs/app-server/README.md) | `app_server_session.rs` · [architecture_zh.md](architecture_zh.md) |
| 生成 schema / TS 类型 | `codex app-server generate-json-schema` / `generate-ts`（见 app-server README） | `codex-app-server-protocol` |
| **CLI slash 命令**（用户可见列表） | [CLI Slash commands](https://developers.openai.com/codex/cli/slash-commands) | [tui-commands_zh.md](tui-commands_zh.md) · `slash_command.rs` |
...
| 文档 | 链接 |
| ---- | ---- |
| TUI 指令（slash / 快捷键） | [tui-commands_zh.md](tui-commands_zh.md) |
| 架构地图 | [architecture_zh.md](architecture_zh.md) |
| 分层详解 | [layered-design_zh.md](layered-design_zh.md) |
